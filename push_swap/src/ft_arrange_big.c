...
/*   Updated: 2022/11/28 19:15:11 by dluna-lo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_find_smaller_chunk(t_list *list, int chunk, int number)
{
	...
	i = 1;
	while (ft_gnl(list, i) <= ((number / 2) - chunk) || ft_gnl(list,
			i) >= ((number / 2) + chunk))
	{
		i++;
	}
	...
	while (ft_gnl(list, ii) <= ((number / 2) - chunk)
		|| ft_gnl(list, ii) >= ((number / 2) + chunk))
	{
		ii--;
	}
	last = ft_gnl(list, ii);
...
void	ft_big_to_b(t_state *state)
{
	...
	{
		state->next = ft_find_smaller_chunk(state->l_a, state->chunk,
				state->max_number);
