...
/*   Updated: 2022/11/11 17:41:30 by diegofranci      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check_parameters(int argc, char *argv[], char *envp[], v_state *state)
{
	...
	if (!ft_strncmp("here_doc", argv[1], 9))
		...
		state->here_doc = 0;
	if (access(argv[1], F_OK) == -1 && state->here_doc != 1)
		ft_error_message("Error ./pipex - 2.1", 1);
	while (i < argc - 1)
	{
		if ((ft_strlen(argv[i]) < 2 || (access(argv[i], F_OK) != -1 && access(argv[i], F_OK) != 0)) && state->here_doc != 1)
